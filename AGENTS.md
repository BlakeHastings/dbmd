...
point, and it has two commands: `dbmd init` and `dbmd studio`. `src/studio/` is
the studio server and the page it serves, reachable either by importing
`startStudio` or through that subcommand. There is no `check` and no `export`.
