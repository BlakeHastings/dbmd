...
sends anything, the index over it is declared unique, and the second insert
loses. If you add a table to this box, or a new kind of write to one of these,
that key comes with it. This is not a nice-to-have: a duplicated `sale` row
takes 250g of coffee out of stock that never left the building, and it shows up
as a stocktake correction six weeks later with no way to work out where it came
from.
