...
- **Two had never been checked by anything, and both are right.** Run here:
 ...
 line at 502; the 438 and
500 above were read from a working tree carrying the owner’s uncommitted edit,
which shifts every line below it. **Both mistakes are the ones this file keeps a
section about**, a gap that closed while the entry stood and a number nobody else
could see.
...
## 2026-09-08: this file, swept for entries its own work overtook

The revisit conditions in the decision records were swept on 2026-09-07 and
sixteen had fired unread. **This file has the same failure mode and nobody had
looked**: it is 2200 lines of dated entries, several of them written to say a
thing was broken hours before it was fixed, and an entry that still reads as open
is a stale instruction in the file about stale instructions.

Swept. Two entries described a state the same night overtook, and the two were
handled differently by accident rather than by rule:

- **The status line that lied after a create** carries its own correction, added
  by the agent that fixed it, saying it was fixed by ADR 0074 the same night and
  naming the two further paths the original entry did not reach. That is the
  shape to copy.
- **The two documentation blocks nothing checked** did not, so it has one now.
  ADR 0076 guarded both, and the entry stood for hours saying they were one edit
  away from being wrong with nobody to notice.

Everything else holds. The entries about the import grid, the edge identities and
the write refusal each name the record that answered them, because each was
written after the answer rather than before it.

**The rule that falls out is the one the decision records already have.** A
...
- **This file held its own body three times, and this entry is the repair.**
  #220, called "The evidence log, swept for entries its own work overtook", took
  it from 2197 lines to 5885. It was found two days later by accident, while
  checking a documentation branch for internal contradictions, and nothing in
  the review or the gate had looked at it: a duplicated markdown file is not
  ...
  **What this file needs and does not have is a reader.** Every page beside it is
  ...
  page's error blocks are run and their codes asserted, every plain JSON payload
  on four pages is claimed by exactly one test case, and one script resolves
