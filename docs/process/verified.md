...
**Corrected within the day: they are guarded now, and two of the line numbers
above are wrong.** ADR 0076 added both to the `NARRATED` table in
`test/docs/readme.test.ts`, so the paragraph below describing an open gap
describes a closed one. The `refs` fence opens at `README.md:439` with its `# Verified
...
line at 440, and the `export` fence at 501 with its `# Verified
