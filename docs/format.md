...
If `_model.md` is there and is a **directory**, you get a
`model-file-not-a-file` warning instead, and it is a separate code because its
fix is the opposite one: writing a file over a directory fails with `EISDIR`, so
the directory has to be moved aside first. Nothing else is said about it. A
directory at the model root that dbmd does not know is normally an
`unknown-kind-directory` warning, and this one name is exempt, because the
warning above already names the same directory and says what to do with it.
...
| `file-unreadable` | error | A file or a kind directory could not be read. The message says why in words, with the errno beside them: `permission denied (EACCES)`, `no such file or directory (ENOENT)`. The errno is all dbmd prints of the system's error, because the rest of it is an absolute path and [ADR 0006](architecture/decisions/0006-one-cli-three-callers.md) rule 4 wants the same bytes on every machine. | Read the errno rather than assuming permissions. `ENOENT` on a name dbmd had just listed is either a link that resolves to nothing or a file that went away mid-read, and the link is the reproducible one: a dangling `tables/orders.md` raises it, and so does a dangling `tables/` itself, which is the kind-directory half and reads `cannot list the directory`. A delete or a branch changed under the command is the other cause. |
...
| `model-file-not-a-file` | warning | There is a `_model.md` and it is a directory, so the model has no name and no engine and no file can be written at that name. Separate from `model-file-missing` because the fix is the opposite one: `model-file-missing` says to write the file, and writing it here fails with `EISDIR`. | Move the directory out of the model root, then write `_model.md` with `kind: model`, a `name:` and an `engine:`. Aside inside the root is not far enough: this name is the only directory at the root that is exempt from `unknown-kind-directory`, and a leading `.` does not exempt one there the way it does inside `tables/`. |
| `unknown-kind-directory` | warning | A directory that is not `tables`, `notes` or `groups`. | Delete the directory, or move it out of the model root. Moving only its files out is not enough: the warning is about the directory, and an empty one that never held a file raises it too. |
...
| `object-not-a-file` | error | A `*.md` name inside `tables/`, `notes/` or `groups/` is a directory rather than a file, so the object it names was not read. A symlinked object file is fine and is followed; this is about one that resolves to a directory, or that is one. | Point the link at the file rather than at the directory holding it. A directory that is not meant to be an object belongs under a name that does not end in `.md`, and under one starting with `.` if it holds markdown of its own: renaming `archive.md/` to `archive/` with an `old.md` inside it trades this error for `object-in-subdirectory`, and `.archive/` clears both. |
...
| `kind-mismatch` | error | `kind:` disagrees with the place the file is in: the directory, for an object file, and the name `_model.md` for the model file. An object file is not loaded; `_model.md` is, and its `name:` and `engine:` are read. | Fix the key, or move the file. Only the key for `_model.md`: that name is what makes it the model file, so moving it leaves a model with no model file and the `model-file-missing` warning that says so. |
...
| `field-wrong-type` | error | A key holds the wrong sort of value: a number where a string was wanted, a scalar where a mapping or a list was, or a key that is not a plain name. | Read the message, which names the sort of value the key takes. Where it wanted a string the answer is usually quotes, and [the quoting rule](#defaults-and-the-quoting-rule) is why. Where it wanted a mapping or a list, quoting changes nothing: `layout: 40` is still not a mapping as `layout: "40"`, and `columns: id` is still not a list as `columns: "id"`. |
...
| `empty-value` | warning | A required name or list is there and says nothing: `name: ""`, a name that is only whitespace, an index whose `columns` is `[]`, or a table file whose name is blank. | Fill it in, or delete the row. The blank file name is the one with no row to delete: rename the file, and its `table:` key with it, which is what that message says. It is a warning because it is what a half-written model looks like; `--strict` fails the run on it. |
...
| `ref-malformed` | error | A `ref:` that is not `table.column`. | Add whichever half is missing, and the message quotes what you wrote so you can tell which. `ref: customers` is short a column; `ref: id` is short a table, and appending `.id` to it gives `ref: id.id` and a `ref-table-unknown` about a `tables/id.md` nobody meant. |
...
| `duplicate-table` | error | Two tables in one model under one name. **Neither way of building a model on disk reaches it.** A directory holds one `orders.md`, and an import of two schemas refuses the collision first: `sales.orders` and `web.orders` produce an `import/name-collision` error, one table written, exit 1, and a `dbmd check` over the result that is clean. What is left is a `Model` a program built itself and handed to the exported `validate`. | Rename one of them. If it came from `dbmd import`, this is not the diagnostic you got: that run said `import/name-collision` and told you to import one schema at a time, and that is the message to act on. |
