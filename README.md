...
**dbmd is not on npm yet**, so that command is what it will be rather than what
you can type today. Until it is published, clone this repository and run
`npm ci && npm run build`, then `node dist/cli.js studio`.
[`CONTRIBUTING.md`](CONTRIBUTING.md) walks that through from a clean clone.
...
**Status: in development.** The four commands below all work from a checkout.
`dbmd init` writes a `db-model/`
directory with a small example model in it, two tables and a sticky note, prose
and all. `dbmd studio` works too: it serves that directory on loopback and
opens it, and `--port N` and `--no-open` are the two flags it takes.
`dbmd check` is the one to put in CI: it reads the model, prints every
...
exit code in a shape a script can read. `dbmd export` writes the model as a
